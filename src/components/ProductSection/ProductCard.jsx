    ...
    <article className={styles.card}>
      <div className={styles.cardImage}>
        <Link to={`/product/${slug}`} className={styles.cardImageLink}>
          <LazyImage src={image} alt={product.name} className={styles.cardImg} />
          <div className={styles.cardImageOverlay} />
          {tag && inStock && <span className={styles.cardTag}>{tag}</span>}
          {discount && inStock && <span className={styles.cardDiscount}>−{discount}%</span>}
          {!inStock && <div className={styles.outOfStockOverlay}><span>Нет в наличии</span></div>}
        </Link>
        <button
          type="button"
          className={`${styles.cardFavBtn} ${favorite ? styles.cardFavBtnActive : ''}`}
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
        ...
        >
          <IconHeart size={16} fill={favorite ? 'currentColor' : 'none'} />
        </button>
      </div>
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.cardFooter}>
          <div className={styles.priceBlock}>
            {oldPrice && (
              <span className={styles.cardOldPrice}>{formatPrice(oldPrice)} ₽</span>
            )}
          ...
          {!inStock ? (
            ...
            <div className={styles.qtyWrap}>
              <button type="button" className={styles.qtyBtn} onClick={remove}>−</button>
              <span className={styles.qtyVal}>{qty}</span>
              <button type="button" className={styles.qtyBtn} onClick={inc}>+</button>
            </div>
          )}
        </div>
      </div>
