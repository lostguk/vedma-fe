...
import { IconUser, IconHeart, IconBag, IconX, IconSearch } from '../Icons'
  ...
  return (
    <>
      <header className={styles.header}>
        ...
        <div className={`${styles.midBar} ${scrolled ? styles.midBarSticky : ''}`}>
        ...
        <div className={`${styles.catBar} ${scrolled ? styles.catBarSticky : ''}`}>
          <div className={`container ${styles.catBarInner}`} ref={catalogRef}>
            ...
            <div className={styles.catBarCenter}>
              ...
              <button
                ...
                onClick={() => {
                  setSearch('')
                  if (location.pathname === '/catalog' && new URLSearchParams(location.search).has('search')) {
                    navigate('/catalog')
                  }
                }}
                aria-label="Очистить"
            ...
            <div className={styles.catBarActions}>
              {isAuthenticated ? (
                <Link to="/profile" className={styles.actionBtn} aria-label="Личный кабинет">
                  <IconUser size={18} />
                  <span className={styles.actionLabel}>Кабинет</span>
                </Link>
              ) : (
                <button className={styles.actionBtn} onClick={() => setLoginOpen(true)} aria-label="Войти">
                  <IconUser size={18} />
                  <span className={styles.actionLabel}>Войти</span>
                </button>
              )}
              <Link to="/favorites" className={styles.actionBtn} aria-label="Избранное" title="Избранное">
                <IconHeart size={18} />
              ...
              <button className={styles.actionBtn} onClick={openDrawer} aria-label="Корзина">
                <IconBag size={18} />
                <span className={styles.actionLabel}>Корзина</span>
                {totalItems > 0 && (
                  <span className={`${styles.actionBadge} ${badgeBump ? styles.actionBadgeBump : ''}`}>
