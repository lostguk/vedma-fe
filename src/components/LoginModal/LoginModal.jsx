  ...
  const handleSubmit = async (e) => {
    ...
    if (view === VIEW_REGISTER) {
      ...
      try {
        ...
        toast.success(name ? `С возвращением, ${name}!` : 'Вы успешно вошли в аккаунт')
        onAuthSuccess?.()
        onClose()
      } catch (err) {
      ...
      <form className={styles.form} onSubmit={handleSubmit} noValidate>
        ...
        {view === VIEW_REGISTER && registerSuccess && (
          <div className={styles.resetSuccess}>
            ...
            <p className={styles.resetSuccessText}>
              На <strong>{email.trim().toLowerCase()}</strong> отправлено письмо для подтверждения.
              Проверьте почту и папку «Спам».
            </p>
            <Button type="button" variant="primary" size="lg" fullWidth onClick={() => switchView(VIEW_LOGIN)}>
              Перейти ко входу
            </Button>
          </div>
        )}
        ...
        {view === VIEW_FORGOT && resetSent && (
          <div className={styles.resetSuccess}>
            ...
            <p className={styles.resetSuccessText}>
              Инструкция по восстановлению пароля отправлена на <strong>{email.trim().toLowerCase()}</strong>.
              Проверьте почту и папку «Спам».
            </p>
            <Button type="button" variant="primary" size="lg" fullWidth onClick={() => switchView(VIEW_LOGIN)}>
              Вернуться ко входу
        ...
        {view === VIEW_FORGOT && !resetSent && (
          <>
            ...
            <button type="button" className={styles.switchBtn} onClick={() => switchView(VIEW_LOGIN)}>
              Вернуться ко входу
            </button>
          </>
        )}
        ...
        {/* Login / Register */}
        {view !== VIEW_FORGOT && !(view === VIEW_REGISTER && registerSuccess) && (
          <>
            {view === VIEW_REGISTER && (
              <>
                <div className={styles.field}>
                  <label className={styles.label} htmlFor="login-firstname">Имя</label>
                  <input
                    id="login-firstname"
                    type="text"
                ...
                <div className={styles.field}>
                  <label className={styles.label} htmlFor="login-lastname">Фамилия</label>
                  <input
                    id="login-lastname"
                    type="text"
                ...
                <div className={styles.field}>
                  <label className={styles.label} htmlFor="login-middlename">Отчество</label>
                  <input
                    id="login-middlename"
                    type="text"
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                autoComplete={view === VIEW_REGISTER ? 'new-password' : 'current-password'}
                disabled={submitting}
              />
            </div>
            ...
            {view === VIEW_REGISTER && (
              <div className={styles.field}>
                <label className={styles.label} htmlFor="login-password2">Повторите пароль</label>
                <input
                  id="login-password2"
                  type="password"
                  placeholder="Ещё раз"
            ...
            {view === VIEW_REGISTER && (
              <label className={styles.consent}>
                ...
                <span className={styles.consentText}>
                  Я соглашаюсь с{' '}
                  <Link to="/privacy" className={styles.consentLink} onClick={onClose}>
            ...
            {view === VIEW_LOGIN && (
              <button type="button" className={styles.forgotBtn} onClick={() => switchView(VIEW_FORGOT)}>
                Забыли пароль?
              </button>
            )}
            ...
            <button
              ...
              onClick={() => switchView(view === VIEW_REGISTER ? VIEW_LOGIN : VIEW_REGISTER)}
            >
              {view === VIEW_REGISTER ? 'Уже есть аккаунт? Войти' : 'Нет аккаунта? Зарегистрироваться'}
            </button>
          </>
        )}
      </form>
